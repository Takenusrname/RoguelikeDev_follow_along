...
use bracket_lib::{random::RandomNumberGenerator, terminal::{DistanceAlg, LineAlg, Point, line2d}};
        ...
        for (i, room) in rooms.iter().enumerate() {
            ...
            for (j, other_room) in rooms.iter().enumerate(){
                if i != j && !connected.contains(&j) {
                    ...
                    let dist = DistanceAlg::Pythagoras.distance2d(room_center_pt, other_center_pt);
                    room_dist.push((j, dist));
                }
            }
            ...
            if !room_dist.is_empty() {
                ...
                let line = line2d(LineAlg::Bresenham, room_center_pt, Point::new(dest_center.0, dest_center.1));
