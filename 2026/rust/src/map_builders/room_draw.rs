...
use bracket_lib::{random::RandomNumberGenerator, terminal::{DistanceAlg, Point}};
    ...
    fn circle(&mut self, build_data: &mut BuilderMap, room: &Rect) {
        ...
        for y in room.y1..=room.y2 {
            for x in room.x1..=room.x2 {
                let idx = build_data.map.xy_idx(x, y);
                let dist = DistanceAlg::Pythagoras.distance2d(center_pt, Point::new(x, y));
                if idx > 0 && idx < ((build_data.map.width * build_data.map.height) - 1) as usize && dist <= radius {
                    build_data.map.tiles[idx] = TileType::Floor;
                }
            }
        }
